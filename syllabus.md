...
- **Data Structures** — multi-dimensional array, list, hash table, string, linked list, queue, stack
- **Graphs** — tree, breadth-first search, depth-first search, topological sort, union find
- **Dynamic Programming & Counting** — top-down (DFS with memoization/caching), bottom-up, DP runtime/space analysis
- **Sorting & Two-Pointer** — binary search, when sorting first makes sense, heap, two-pointer algorithms
- **Other topics** not specifically tested in topic exams: databases, concurrency, counting, prefix sum, math, ML/AI ([deep-ml.com](https://www.deep-ml.com/problems))
...
**You must pass the final exam to pass the course.** The target is to solve any
medium-level question with roughly a 40%–60% acceptance rate.

- **12 questions** over **5½ hours**.
