# Week 13 - Sort, Order

## Class

- [Removing Minimum and Maximum from Array](https://leetcode.com/problems/removing-minimum-and-maximum-from-array/) *(in class)*
- [Minimum Area Rectangle](https://leetcode.com/problems/minimum-area-rectangle/) *(in class)*
- [Count Elements with Strictly Smaller and Greater Elements](https://leetcode.com/problems/count-elements-with-strictly-smaller-and-greater-elements/) *(recommended)*
- [Distant Barcodes](https://leetcode.com/problems/distant-barcodes/)

## Outside of Class

- [Interview Ready extension](https://chromewebstore.google.com/detail/interview-ready/llicapjcmjifdbafkbicgplkmamfchmd?authuser=1&hl=en) *(Maybe use the extension to practice?)*

## Online Assessment from last week 😊

- [How Many Numbers Are Smaller Than the Current Number](https://leetcode.com/problems/how-many-numbers-are-smaller-than-the-current-number/)
- [Merge Sorted Array](https://leetcode.com/problems/merge-sorted-array/)
- [Container with Most Water](https://leetcode.com/problems/container-with-most-water/)
- [Merge Intervals](https://leetcode.com/problems/merge-intervals/)
- [Maximum Length of Pair Chain](https://leetcode.com/problems/maximum-length-of-pair-chain/)
- [Count Complete Tree Nodes](https://leetcode.com/problems/count-complete-tree-nodes/) Runtime less than O(N)
- [Minimum Number of Arrows to Burst Balloons](https://leetcode.com/problems/minimum-number-of-arrows-to-burst-balloons/)
- [Sort Colors](https://leetcode.com/problems/sort-colors/) Runtime O(N), Space O(1)
- [Sort List](https://leetcode.com/problems/sort-list/)
- [Largest Divisible Subset](https://leetcode.com/problems/largest-divisible-subset/)
- [Task Scheduler](https://leetcode.com/problems/task-scheduler/)
- [Number of Atoms](https://leetcode.com/problems/number-of-atoms/)
- [Minimum Area Rectangle](https://leetcode.com/problems/minimum-area-rectangle/)
- [Search a 2D Matrix](https://leetcode.com/problems/search-a-2d-matrix/) Runtime O(log(m * n)) or better, Space O(1)
- [Minimum Score by Changing Two Elements](https://leetcode.com/problems/minimum-score-by-changing-two-elements/)
- [Maximize Greatness of an Array](https://leetcode.com/problems/maximize-greatness-of-an-array/)
- [Design a Number Container System](https://leetcode.com/problems/design-a-number-container-system/)
- [Sort an Array](https://leetcode.com/problems/sort-an-array/)
- [Furthest Building You Can Reach](https://leetcode.com/problems/furthest-building-you-can-reach/)
- [Sort List](https://leetcode.com/problems/sort-list/)
- [Sort an Array](https://leetcode.com/problems/sort-an-array/)
- [Furthest Building You Can Reach](https://leetcode.com/problems/furthest-building-you-can-reach/)

## Reflection

> Consider journaling the answers to these questions. What guidance in your career and life search have you prayed for this week? What guidance have you been given this week?

## This week

- 📅 [Sorting, Two-Pointer, etc. Performance](../topic-exams/sorting-two-pointer/live-performance-exam.md)

## Learning objectives

- Identify when sorting helps
